...
| 功能 | 说明 |
| --- | --- |
| 桌面窗口 | dsh 完整界面装进原生窗口,启动直达,无需浏览器 |
| 系统托盘 | 常驻托盘,后台继续运行任务,点击即回 |
| 系统通知 | 任务完成、需要确认时收到桌面通知 |
| 右键菜单 | 常用操作集成到原生右键菜单 |
| 自动更新 | 应用与 dsh 内核分别自动更新,无需重装 |
| 诊断导出 | 遇到问题一键导出日志,方便求助与反馈 |

桌面增强全部以 dsh 官方插件形式实现,不喜欢可以在 dsh 里关闭,关闭后不影响核心功能。
